...
def read_vtk_file(filename: str) -> Optional[Dict[str, Any]]:
    ...
    Returns:
        Dictionary with keys: x, y, data_fields, nx, ny, dx, dy, filename
...
def calculate_flow_metrics(data):
    ...
    metrics["velocity_uniformity"] = metrics["velocity_std"] / (
        metrics["mean_velocity"] + 1e-10
    )
...
def compare_two_cases(data1, data2, case1_name, case2_name, output_dir):
    ...
    ax7.plot(
        data1["x"], u1[y_center_idx1, :], "b-", linewidth=2, label=f"{case1_name} u-vel"
    )
    ax7.plot(
        data2["x"], u2[y_center_idx2, :], "r-", linewidth=2, label=f"{case2_name} u-vel"
    )
    ax7.set_xlabel("x (m)")
    ...
    ax9.hist(
        velocity_mag1.flatten(), bins=50, alpha=0.7, label=case1_name, density=True
    )
    ax9.hist(
        velocity_mag2.flatten(), bins=50, alpha=0.7, label=case2_name, density=True
    )
    ...
    for metric in sorted(all_metrics):
        ...
        if isinstance(val1, (int, float)) and isinstance(val2, (int, float)):
            ...
            stats_data.append(
                [metric.replace("_", " ").title(), str(val1), str(val2), "N/A", "N/A"]
            )
    ...
    for i in range(len(stats_data)):
        if stats_data[i][4] != "N/A":
            try:
                ...
                pass  # Skip highlighting if percentage can't be parsed
    ...
    output_file = os.path.join(
        output_dir, f"comparison_{case1_name}_vs_{case2_name}.png"
    )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Parameter study and comparison tool for CFD data"
    )
    parser.add_argument(
        "--input_dir",
        "-d",
    ...
    parser.add_argument(
        "--pattern", "-p", help='File pattern to match (e.g., "optimized*.vtk")'
    )
    ...
    if args.compare:
        ...
        # If we have exactly 2 files, do a comparison
            ...
            compare_two_cases(
                data_list[0], data_list[1], case1_name, case2_name, output_dir
            )

        else:
            print(
                f"Found {len(data_list)} files. Use --sweep to analyze parameter variations"
            )
